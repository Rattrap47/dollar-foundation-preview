/* ==========================================================================
   Dollar Foundation — progressive enhancement only.
...
(function () {
  ...
  /* ---- Donate links ------------------------------------------------------
     Every donate button ships pointing at the "How to give" section. A
     configured donation URL replaces it; a missing one leaves the fallback.
  ------------------------------------------------------------------------ */
  if (CFG.donateUrl) {
  ...
  function mailtoFallback(v) {
    if (!CFG.contactEmail) {
      setStatus('This form is not connected yet. For now, please call us on +233 24 603 0338 ' +
                'or send a message to our Facebook page.', 'err');
      return;
    }
  ...
  form.addEventListener('submit', function (e) {
    ...
    fetch(CFG.contactEndpoint, {
      ...
      .then(function (res) {
        ...
        form.reset();
        setStatus('Thank you — your message is on its way. Someone from the foundation will get back to you.', 'ok');
      })
      .catch(function () {
        setStatus('Sorry, that did not go through. Please call us on +233 24 603 0338 instead.', 'err');
      })
      .then(function () {
        if (submitBtn) {
